 ...
 *      并且顺带能搜在售楼盘,不再只有区域。
 *   3. **选了真的会带你去** —— 以前只 flyTo 一下,地图静静飞过去,没有任何「到了」
 *      的反馈;现在落地即打开该区详情 / 该楼盘。
 *
...
export default function MapSearch({
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); const s = results[active]; if (s) pick(s) }
  }
  ...
  }, [showHint, loading, results, active, query, t])
