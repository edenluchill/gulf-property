...
interface VisualContentSectionProps {
  ...
  hiddenProjectImages?: string[]
  visualContent?: VisualContent
  isProcessing: boolean
  primaryImage?: string
  onPrimaryImageChange?: (imageUrl: string | undefined) => void
  onProjectImagesChange?: (images: string[]) => void
  onHiddenProjectImagesChange?: (hidden: string[]) => void
}
  ...
  return (
    <div className="space-y-4 pt-6 border-t-2 border-gray-100">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-lg font-bold text-gray-900">{t('visualContent.title')}</h3>
          <p className="text-sm text-gray-600">{t('visualContent.subtitle')}</p>
        </div>
      </div>

      {/* Project Images with Sortable Grid */}
      {hasProjectImages && (
        <div className="space-y-3 p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-200">
          <SortableImageGrid
            images={projectImages}
            hiddenImages={hiddenProjectImages}
            onImagesChange={onProjectImagesChange || (() => {})}
            onHiddenImagesChange={onHiddenProjectImagesChange}
            primaryImage={primaryImage}
            onPrimaryImageChange={onPrimaryImageChange}
            label={t('visualContent.projectImages', '项目图片')}
          />
        </div>
      )}

      {/* Visual Content Detection (when images not extracted) */}
      {!hasProjectImages && hasVisualContent && (
