 ...
 * 高空俯瞰的俯角。
 *
 * 这个值来回调过三次，每次都是**只看了一头**。完整的账是：
 *   • 太平（20°）→ 构图准，但 owner:「感觉是**直勾勾从上而下**观望」，没有航拍味。
 *   • 太陡（45°+ 而且构图不管）→ 近景地面吃掉下半屏，主角被挤到上面三分之一。
 *
 * 关键是**这两件事其实不冲突** —— 冲突的是「陡」和「乱瞄」。开场机位是按几何算出来的，
 * 用 `scripts/tour-framing.mjs` 量过:pitch 20 时主角落在可见区的 y≈0.49（正中），
 * **说明还有大量余量往陡了调**。现在 38°/46°:明显是斜的、有航拍味，主角仍在中间。
 *
 * ⚠️ **改这个值必须重跑 `scripts/tour-framing.mjs`** —— 它量的是主角的归一化坐标，
 *    肉眼看截图是判断不出「有没有被挤出可见区」的。
 * ⚠️ 俯角越大，朝地平线铺出去的远景瓦片越多（手机上最贵的一项）。要再陡先看跑分。
 */
const openingPitch = () => (isNarrowViewport() ? 38 : 46)
/** 项目群四周留的余量(1 = 贴边)。 */
