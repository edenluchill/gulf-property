 ...
 *
 * owner 第二次实测后点破的:
 *   「理应客户应该看到**同样深度** —— 比如经纪看到增长率,那买家也要看到呀。
 *     **不在一个 zoom level 怎么介绍附近增长率?**」
 *
 * 这句话推翻了我前两版的整个思路:
 *   v1「客户必须看全经纪的画面」→ iPad→手机要缩 1.6 级 → **内容只剩 33%,全看不清**
 *   v2「那就少缩一点」(0.5 级)  → **还是在缩** —— 而只要缩,标签密度就对不上:
 *      经纪说「你看这个区涨了 15%」,客户屏幕上**根本没有那个数字**。
 *
 * 地图的 zoom **不只是缩放,它决定显示什么** —— 区域标签、涨幅 chip、POI 在低 zoom
 * 会被抽稀甚至整个消失。所以「同一个 zoom」不是审美问题,是**两个人能不能在讲同一件事**。
 *
 * 现在:客户**照抄经纪的 zoom**(同样深度、同样标签),屏幕小就只看到中间那一块 ——
 * 而经纪讲的东西**本来就在中间**。要让他看更大范围,经纪自己 zoom out,客户跟着一起 out。
 */
const MAX_SHRINK = 0
/**
 * 但**放大**是要的:经纪在手机(390 宽)、客户在电脑(1440 宽)时照抄 zoom,
 * 东西会小得可笑 —— 这时把 zoom 提上去,让**屏幕上的观感尺寸**对齐。
 */
const MAX_GROW = 1.5
