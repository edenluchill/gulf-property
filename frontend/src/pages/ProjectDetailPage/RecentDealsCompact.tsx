          ...
          <table className="w-full text-sm" style={{ minWidth: 440 }}>
            <thead>
              <tr className="border-b text-[11px] uppercase tracking-wide text-slate-400">
                <th className="py-2 text-start pe-3 font-medium">{tk('colDate')}</th>
                <th className="py-2 text-start pe-3 font-medium">{tk('colUnit')}</th>
                <th className="py-2 text-end px-2 font-medium">{tk('colSize')}</th>
                <th className="py-2 text-end px-2 font-medium">{tk('colPrice')}</th>
                <th className="py-2 text-end ps-2 font-medium">AED/m²</th>
              </tr>
            </thead>
            <tbody>
              {sales.map((s, i) => (
                <tr key={i} className="border-b last:border-0 text-slate-700">
                  <td className="py-2.5 pe-3 tabular-nums text-slate-500">{s.date || '—'}</td>
                  <td className="py-2.5 pe-3">
                    <span className="truncate">{s.building || '—'}</span>
                    {s.rooms && <span className="ms-1.5 text-[11px] text-slate-400">{s.rooms}</span>}
                    <span className={`ms-1.5 rounded px-1 py-0.5 text-[10px] ${s.saleType === 'offplan' ? 'bg-violet-50 text-violet-600' : 'bg-slate-100 text-slate-500'}`}>
                      {s.saleType === 'offplan' ? tk('offplan') : tk('ready')}
                    </span>
                  </td>
                  <td className="py-2.5 px-2 text-end tabular-nums text-slate-500">{s.sizeSqm ? `${s.sizeSqm}m²` : '—'}</td>
                  <td className="py-2.5 px-2 text-end tabular-nums">{s.price ? <><DirhamSymbol size="0.7em" className="text-slate-400" />{formatMoneyCompact(s.price, lang)}</> : '—'}</td>
                  <td className="py-2.5 ps-2 text-end tabular-nums">{s.pricePerSqm ? s.pricePerSqm.toLocaleString() : '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
