/**
 * About / Features — Dark-Luxury marketing + SEO + AI-discoverability page.
 *
 * Cohesive dark aesthetic (slate-950 base, teal #00E0B8 + gold accents, glass
 * cards, soft glows), modern scroll-reveal animations (framer-motion whileInView),
 * inline SVG/CSS diagrams (no external assets beyond the hero product shot).
 * Helmet adds meta + JSON-LD so search engines and AI crawlers understand Pinzos.
 */
...
// scroll-reveal: fade + rise when it enters the viewport (once)
    ...
    '@graph': [
      { '@type': 'Organization', name: 'Pinzos', url: 'https://pinzos.com', description: 'Interactive Dubai off-plan property platform with real DLD data, AI brochure parsing, investment analytics, and agent tools for overseas clients.', areaServed: 'Dubai, United Arab Emirates' },
      {
        ...
        offers: [
          ...
          { '@type': 'Offer', name: 'Agent', price: '199', priceCurrency: 'USD' },
        ...
        featureList: [
          ...
          'Filters by price, bedrooms, status, developer', 'POI layers and distance measuring',
    ...
    <div className="flex-1 overflow-y-auto bg-slate-950 text-white">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-12 px-6 py-16 md:py-24 lg:grid-cols-2">
          <Reveal>
            ...
            <p className="mt-5 max-w-xl text-lg leading-relaxed text-slate-300">
              {L('一张交互式卫星地图,装下迪拜每个区的真实成交、租金与回报;AI 读懂开发商楼书;经纪能带海外客户实时在地图上看房、生成 AI 导览与意向报告。',
      ...
      <nav className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/85 backdrop-blur">
        <div className="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-4 py-2.5 text-sm">
          {([
            ['map', L('地图情报', 'Map intelligence')], ['investment', L('投资分析', 'Investment')],
            ['ai', L('AI 解析', 'AI parsing')], ['agents', L('经纪工具', 'For agents')], ['pricing', L('定价', 'Pricing')],
      ...
      {/* ── Two sides ───────────────────────────────────────── */}
      <Section>
        <Reveal><SectionTitle eyebrow={L('一个平台,两端受益', 'One platform, two sides')} title={L('给买家洞察,给经纪生意', 'Insight for buyers. Business for agents.')} /></Reveal>
        <div className="grid gap-5 md:grid-cols-2">
          <Reveal><SideCard accent title={L('买家 / 投资人', 'Buyers / Investors')} desc={L('在一张地图上看懂迪拜每个区值不值得买。', 'Understand every Dubai district on one map.')}
            items={[L('真实成交 + 租约', 'Real sales + rents'), L('5 年回报 + 黄金签证', '5-yr ROI + Golden Visa'), L('户型/付款计划', 'Units / payment plans')]} /></Reveal>
          <Reveal delay={0.1}><SideCard title={L('经纪 / 团队', 'Agents / Teams')} desc={L('把地图变成成交工具:带看、导览、报告。', 'Turn the map into a closing tool.')}
            items={[L('实时海外带看', 'Live overseas tours'), L('Luna AI 智能导览', 'Luna AI tours'), L('买家意向报告', 'Buyer-intent reports')]} /></Reveal>
        </div>
      </Section>

      {/* ── Map intelligence ────────────────────────────────── */}
      <Section id="map" glow>
        <Reveal><SectionTitle eyebrow={L('地图情报', 'Map intelligence')} icon={<MapIcon className="h-5 w-5" />} title={L('整个迪拜,真实数据,一眼看懂', 'All of Dubai, in real data, at a glance')} /></Reveal>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[
            { icon: <TrendingUp />, t: L('最新成交 & 租约', 'Latest sales & rents'), d: L('每个区直接看 DLD 真实成交价与租约,不是估算。', 'Real DLD prices and rental contracts per district — not estimates.') },
            { icon: <Database />, t: L('区域指标', 'Area metrics'), d: L('中位房价、价/sqft、资本增长、租金回报、租赁稳定性,一键切换。', 'Median price, price/sqft, growth, yield and stability — one tap.') },
            { icon: <Layers />, t: L('筛选 & 图层', 'Filters & layers'), d: L('按价格/卧室/状态/开发商筛;叠加交通、医院、学校、超市。', 'Filter by price/beds/status/developer; overlay transit, hospital, school, supermarket.') },
            { icon: <Ruler />, t: L('测距 & 3D', 'Measure & 3D'), d: L('量到地铁/海滩/机场的距离;3D 倾斜 + 地标扣图。', 'Measure distance to metro/beach/airport; 3D tilt + landmark cut-outs.') },
            { icon: <Building2 />, t: L('项目详情', 'Project detail'), d: L('户型、面积(㎡/sqft)、付款计划、配套、位置。', 'Unit types, area (m²/sqft), payment plans, amenities, location.') },
            { icon: <KeyRound />, t: L('投资信心', 'Investor confidence'), d: L('黄金签证资格、永久产权、零房产税/资本利得税。', 'Golden-Visa eligibility, freehold, zero property & capital-gains tax.') },
          ].map((f, i) => (
            <Reveal key={i} delay={(i % 3) * 0.07}><FeatureCard icon={f.icon} title={f.t} desc={f.d} /></Reveal>
          ))}
        </div>
      </Section>
      ...
      <Section id="investment">
        <Reveal><SectionTitle eyebrow={L('投资分析', 'Investment analytics')} icon={<TrendingUp className="h-5 w-5" />} title={L('不止看价格,看 5 年后值多少', 'Beyond price — what it\'s worth in 5 years')} /></Reveal>
        <div className="grid items-center gap-8 md:grid-cols-2">
          <Reveal><p className="leading-relaxed text-slate-300">
            {L('每个项目都用真实区域成交数据测算:租金回报率、5 年增值、年化回报、回本年限 — 全部标注数据来源,不编造。再加黄金签证门槛判定与免税优势,海外买家最关心的问题一次答清。',
              'Every project is modelled on real area DLD data: rental yield, 5-year appreciation, annualized return and payback — each figure cites its source, never invented. Plus Golden-Visa thresholds and tax-free advantages.')}
          </p></Reveal>
          <div className="grid grid-cols-2 gap-3">
            {[
              { k: L('租金回报', 'Rental yield'), v: '~6–8%' }, { k: L('5 年年化', '5-yr annualized'), v: '↑' },
              { k: L('回本年限', 'Payback'), v: '~12y' }, { k: L('黄金签证', 'Golden Visa'), v: 'AED 2M+' },
            ].map((m, i) => (
              <Reveal key={i} delay={(i % 2) * 0.08}>
                <div className="rounded-2xl bg-white/[0.04] p-4 ring-1 ring-white/10">
                  <div className="text-2xl font-bold" style={{ color: ACCENT }}>{m.v}</div>
                  <div className="mt-1 text-xs font-medium text-slate-400">{m.k}</div>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <Section id="ai" glow>
        <Reveal><SectionTitle eyebrow={L('AI 楼书解析', 'AI brochure parsing')} icon={<Sparkles className="h-5 w-5" />} title={L('上传一份楼书,自动变成结构化房源', 'Upload a brochure → a structured listing')} /></Reveal>
        <Reveal delay={0.1}><Pipeline steps={[
          { icon: <Upload className="h-5 w-5" />, t: L('上传开发商 PDF', 'Upload developer PDF') },
          { icon: <Sparkles className="h-5 w-5" />, t: L('AI 提取户型/价格/付款计划/配套', 'AI extracts units / prices / plans / amenities') },
          { icon: <Building2 className="h-5 w-5" />, t: L('结构化房源上线地图', 'Structured listing on the map') },
        ]} /></Reveal>
        <Reveal delay={0.2}><p className="mt-6 text-center text-sm text-slate-400">
          {L('几百页的楼书几分钟变成可搜索、可比较、可上地图的房源数据。', 'Hundreds of brochure pages become searchable, comparable, map-ready listing data in minutes.')}
        </p></Reveal>
      </Section>

      {/* ── Agent: live co-presence tour ───────────────────── */}
      <Section id="agents">
        <Reveal><SectionTitle eyebrow={L('经纪变现 · 招牌功能', 'For agents · flagship')} icon={<Radio className="h-5 w-5" />} title={L('带海外客户,实时在地图上看房', 'Tour overseas clients live, on the map')} /></Reveal>
        <Reveal delay={0.06}><p className="mb-8 max-w-3xl leading-relaxed text-slate-300">
      ...
      <Section glow>
        <div className="grid gap-5 md:grid-cols-2">
          <Reveal><BigCard icon={<FileText />} title={L('Luna AI 智能导览', 'Luna AI guided tour')}
            desc={L('为客户生成一条可分享的自助看房导览:AI 精选房源、语音讲解、5 年回报测算。客户随时打开自己看,行为(看了哪些区/停留多久/问了什么)回传给经纪。',
              'Generate a shareable self-serve tour: AI-picked properties, voice narration, 5-year ROI. Clients open it anytime; their behaviour flows back to the agent.')} /></Reveal>
          <Reveal delay={0.1}><BigCard icon={<FileText />} title={L('买家意向报告', 'Buyer-intent reports')}
            desc={L('每次带看后自动生成报告:客户看了哪些区/项目、聊了什么、问了 Luna 什么,AI 判定意向等级并给出可直接发的跟进话术。',
              'After each tour, an automatic report: areas/projects viewed, what was discussed and asked, an AI-judged interest level, and a ready-to-send follow-up.')} /></Reveal>
        </div>
      </Section>
      ...
      <Section>
        <Reveal>
          <div className="relative overflow-hidden rounded-3xl bg-white/[0.04] p-8 ring-1 ring-white/10 md:p-12">
            <div className="absolute -right-16 -top-16 h-64 w-64 rounded-full opacity-20 blur-3xl" style={{ background: ACCENT }} />
      ...
      <Section id="pricing" glow>
        <Reveal><SectionTitle eyebrow={L('定价', 'Pricing')} icon={<KeyRound className="h-5 w-5" />} title={L('买家免费,经纪按量选档', 'Free for buyers. Plans for agents.')} /></Reveal>
        <div className="grid items-stretch gap-5 lg:grid-cols-3">
          <Reveal><PriceTier name={L('探索版', 'Explore')} price={L('免费', 'Free')} note={L('给买家 / 投资人', 'For buyers / investors')}
            features={[L('交互式地图 + 真实 DLD 数据', 'Interactive map + real DLD data'), L('区域指标 + 项目详情 + 投资分析', 'Area metrics + detail + analytics'), L('Luna 语音助手', 'Luna voice assistant'), L('经纪工具试用:带看/导览/报告 各 2 次', 'Agent trial: 2 tours / tours / reports each')]}
            cta={{ label: L('打开地图', 'Open the map'), to: '/' }} /></Reveal>
          <Reveal delay={0.08}><PriceTier highlight name={L('经纪版', 'Agent')} price="$199" priceWas="$299" per={L('/ 月', '/ mo')}
          ...
          <Reveal delay={0.16}><PriceTier founder name={L('创始会员', 'Founder')} price="$699" per={L('/ 月', '/ mo')}
            badge={L('5× 额度', '5× quota')} note={L('早期支持者 · 名额有限', 'Early supporters · limited')}
            features={[L('实时带看 100 场/月', '100 live tours / mo'), L('Luna 导览 100 个/月', '100 Luna AI tours / mo'), L('意向报告 150 份/月', '150 reports / mo'), L('锁定创始价 · 优先支持 · 抢先体验', 'Locked price · priority support · early access')]}
            cta={{ label: L('申请 Founder', 'Apply for Founder'), href: 'mailto:info@pinzos.com?subject=Pinzos%20Founder%20Access%20($699)' }} /></Reveal>
        </div>
        <p className="mt-5 text-center text-xs text-slate-500">{L('价格以美元计,可开月付。', 'Prices in USD, billed monthly.')}</p>
      </Section>
...
// ── building blocks (dark-luxury) ───────────────────────────
function Section({ id, glow, children }: { id?: string; glow?: boolean; children: React.ReactNode }) {
  return (
    <section id={id} className="relative scroll-mt-14 border-b border-white/5 py-16">
      {glow && <div className="pointer-events-none absolute right-0 top-1/3 h-72 w-72 rounded-full opacity-[0.07] blur-3xl" style={{ background: ACCENT }} />}
...
function SectionTitle({ eyebrow, title, icon }: { eyebrow: string; title: string; icon?: React.ReactNode }) {
  return (
    <div className="mb-8">
      <div className="mb-2 flex items-center gap-2 text-sm font-semibold" style={{ color: ACCENT }}>{icon}{eyebrow}</div>
...
function FeatureCard({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="h-full rounded-2xl bg-white/[0.04] p-5 ring-1 ring-white/10 backdrop-blur transition hover:-translate-y-0.5 hover:bg-white/[0.07] hover:ring-white/20">
      <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl" style={{ background: 'rgba(0,224,184,0.12)', color: ACCENT }}>{icon}</div>
...
function SideCard({ title, desc, items, accent }: { title: string; desc: string; items: string[]; accent?: boolean }) {
  return (
    <div className={`h-full rounded-2xl p-6 ring-1 ${accent ? 'ring-white/15' : 'ring-white/10'} bg-white/[0.04] backdrop-blur`}>
      <h3 className="text-xl font-bold text-white">{title}</h3>
      <p className="mt-1 text-sm text-slate-400">{desc}</p>
      <ul className="mt-4 space-y-2">
        {items.map((it, i) => (
          <li key={i} className="flex items-center gap-2 text-sm text-slate-200"><span className="h-1.5 w-1.5 rounded-full" style={{ background: accent ? ACCENT : GOLD }} /> {it}</li>
        ))}
      </ul>
    </div>
  )
...
function PriceTier({ name, price, priceWas, per, badge, note, features, cta, highlight, founder }: {
  ...
  features: string[]; cta: CTA; highlight?: boolean; founder?: boolean
}) {
  const edge = founder ? GOLD : ACCENT
  const btn = (
    <span className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-900 transition hover:opacity-90" style={{ background: edge }}>
      {cta.label}
    </span>
  )
  return (
    <div className="relative flex h-full flex-col rounded-2xl bg-white/[0.04] p-6 ring-1 ring-white/10 backdrop-blur"
      style={highlight ? { boxShadow: `0 0 0 2px ${ACCENT}` } : founder ? { boxShadow: `0 0 0 1px ${GOLD}55` } : undefined}>
    ...
    <div className="flex flex-col items-stretch gap-3 md:flex-row md:items-center">
      {steps.map((s, i) => (
        <div key={i} className="flex flex-1 items-center gap-3 md:flex-col md:gap-3 md:text-center">
          <div className="flex w-full items-center gap-3 rounded-2xl bg-white/[0.04] p-4 ring-1 ring-white/10 md:flex-col">
...
function CoPresenceDiagram({ L }: { L: (cn: string, en: string) => string }) {
  const Node = ({ icon, title, sub, accent }: { icon: React.ReactNode; title: string; sub: string; accent?: boolean }) => (
    <div className={`flex-1 rounded-2xl p-5 text-center ring-1 backdrop-blur ${accent ? 'bg-white/[0.07] ring-white/20' : 'bg-white/[0.04] ring-white/10'}`}>
  ...
  const Arrow = ({ label }: { label: string }) => (
    <div className="flex shrink-0 flex-col items-center justify-center px-1 md:px-2">
      <ArrowRight className="hidden h-5 w-5 md:block" style={{ color: ACCENT }} />
