...
export default function ProjectReportPage() {
  ...
  const { t: tRaw, i18n } = useTranslation('projectReport')
  const t = tRaw as T
  ...
  if (state === 'error' || !d) return <div className="flex min-h-screen items-center justify-center text-slate-400">{t('notFound')}</div>
  ...
  const M = (v: number | null | undefined) => v != null ? formatMoneyCompact(v, normLang(i18n.language)) : '—'
    ...
    <div className="min-h-screen bg-slate-50 pb-24 text-slate-900">
      ...
      <div className="relative h-52 w-full overflow-hidden bg-slate-800 sm:h-64">
        ...
        <div className="absolute bottom-0 left-0 right-0 mx-auto max-w-xl px-5 pb-4 text-white">
          <div className="flex items-center gap-1.5 text-xs opacity-90"><MapPin className="h-3.5 w-3.5" />{project.area || t('defaultArea')}</div>
      ...
      <div className="mx-auto max-w-xl px-4">
        ...
        <div className="-mt-7 flex items-center gap-3 rounded-2xl bg-white p-4 shadow-lg ring-1 ring-slate-900/[0.06]">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[11px] font-medium uppercase tracking-wide text-teal-600">{t('agent.yourAdvisor')}</div>
          ...
          {wa && (
            <a href={contactHref} onClick={() => trackEvent('contact_attempt', { contact_type: contactHref.startsWith('https://wa.me/') ? 'whatsapp' : 'phone' }, { project_id: project?.id, immediate: true })} className="flex items-center gap-1.5 rounded-full bg-teal-500 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-teal-600">
              {agent.whatsapp ? <MessageCircle className="h-4 w-4" /> : <Phone className="h-4 w-4" />}{t('agent.consult')}
            </a>
          )}
        </div>
        ...
        <Section>
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
            <Stat label={t('stat.startingPrice')} value={<D v={project.starting_price || project.min_price} />} />
            <Stat label={t('stat.bedrooms')} value={project.min_bedrooms != null ? t('stat.bedroomsValue', { range: `${project.min_bedrooms}${project.max_bedrooms && project.max_bedrooms !== project.min_bedrooms ? `–${project.max_bedrooms}` : ''}` }) : '—'} />
            <Stat label={t('stat.areaAvgSqm')} value={area?.median_price_sqm != null ? <D v={area.median_price_sqm} /> : '—'} />
            <Stat label={t('stat.rentalYield')} value={area?.rental_yield_pct != null ? <span className="text-emerald-600">{Number(area.rental_yield_pct).toFixed(1)}%</span> : '—'} />
          </div>
        </Section>

        {/* Investment — simple, evidence-backed, with chart */}
        {inv && (
          <Section title={t('invest.title')} icon={<TrendingUp className="h-4 w-4 text-teal-500" />}>
            {/* Headline */}
            <div className="mb-3 flex items-end justify-between gap-3 rounded-xl bg-teal-50 px-4 py-3">
              <div>
                <div className="text-[11px] text-teal-700/70">{t('invest.invested')} <DirhamSymbol size="0.7em" />{M(inv.buy)}{' · '}{t('invest.projectedTotal')}</div>
              ...
              <div className="text-end">
                ...
                <div className="text-[11px] text-slate-500">{inv.payback_years != null ? t('invest.annualizedWithPayback', { years: Number(inv.payback_years).toFixed(0) }) : t('invest.annualized')}</div>
              </div>
            </div>

            {/* How it's made — money flow bar */}
            <FlowBar buy={inv.buy} appr={inv.appreciation_5yr} rent={inv.rental_income_5yr} fmt={M} t={t} />
            ...
            <div className="mt-3 flex items-start gap-1.5 rounded-lg bg-slate-50 px-3 py-2 text-[11px] leading-relaxed text-slate-500">
              ...
              <span>
                {t('invest.basis', {
                  area: project.area || t('invest.thisArea'),
                  comps: sales.length ? t('invest.comps', { n: sales.length }) : '',
                  yieldPct: area?.rental_yield_pct != null ? `${Number(area.rental_yield_pct).toFixed(1)}%` : '—',
                  growthPct: area?.price_growth_pct != null ? `${Number(area.price_growth_pct).toFixed(1)}%` : '—',
                })}{' '}
                <b>{t('invest.disclaimer')}</b>
              </span>
            </div>
          </Section>
        )}
        ...
        {series && series.price.some((v) => v != null) && (
          <Section title={t('trend.title')} icon={<TrendingUp className="h-4 w-4 text-teal-500" />} badge={t('trend.badge')}>
            <TrendChart months={series.months} price={series.price} t={t} />
            <p className="mt-1 text-[11px] text-slate-400">{t('trend.note', { area: project.area || t('defaultArea') })}</p>
          </Section>
        )}

        {/* Supply */}
        {supply && supply.units_pipeline > 0 && (
          <Section title={t('supply.title')} icon={<Building2 className="h-4 w-4 text-teal-500" />}>
            <div className="grid grid-cols-3 gap-2.5">
              <Stat label={t('supply.pipelineProjects')} value={`${supply.pipeline_projects}`} />
              <Stat label={t('supply.pipelineUnits')} value={Number(supply.units_pipeline).toLocaleString()} />
              <Stat label={t('supply.handover1y')} value={Number(supply.units_handover_1y).toLocaleString()} />
            </div>
          </Section>
        )}
        ...
        {sales.length > 0 && (
          <Section title={t('deals.title')} icon={<BadgeCheck className="h-4 w-4 text-emerald-500" />} badge={t('deals.badge')}>
              ...
              {sales.slice(0, 5).map((r: any, i: number) => (
                <div key={i} className="flex items-center justify-between gap-3 px-3 py-2.5">
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-slate-400">{r.date}{r.rooms ? ` · ${r.rooms}` : ''}{r.sizeSqm ? ` · ${t('deals.sqm', { n: r.sizeSqm })}` : ''}</div>
                  </div>
        ...
        {insights?.nearby?.metro?.length > 0 && (
          <Section title={t('nearby.title')} icon={<MapPin className="h-4 w-4 text-teal-500" />}>
        ...
        <p className="mt-6 text-center text-[11px] text-slate-400">{t('footer')}</p>
      </div>
      ...
      {wa && (
        <div className="rp-no-print fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 p-3 backdrop-blur">
          <a href={contactHref} className="mx-auto flex max-w-xl items-center justify-center gap-2 rounded-xl bg-teal-500 py-3 font-semibold text-white shadow-sm hover:bg-teal-600">
            {agent.whatsapp ? <MessageCircle className="h-5 w-5" /> : <Phone className="h-5 w-5" />}{t('agent.consultMore', { name: agent.name })}
          </a>
        </div>
      )}
    </div>
...
function FlowBar({ buy, appr, rent, fmt, t }: { buy: number; appr: number | null; rent: number | null; fmt: (v: number | null) => string; t: T }) {
  ...
  return (
    <div>
      <div className="flex h-7 w-full overflow-hidden rounded-lg text-[10px] font-semibold text-white">
        <div className="flex items-center justify-center bg-slate-400" style={{ width: seg(buy) }}>{t('invest.flowPrincipal')}</div>
        {a > 0 && <div className="flex items-center justify-center bg-teal-500" style={{ width: seg(a) }}>{t('invest.flowAppreciation')}</div>}
        {r > 0 && <div className="flex items-center justify-center bg-emerald-500" style={{ width: seg(r) }}>{t('invest.flowRent')}</div>}
      </div>
      <div className="mt-2 grid grid-cols-3 gap-2 text-center">
        <Leg color="bg-slate-400" label={t('invest.legPrincipal')} value={<><DirhamSymbol size="0.7em" className="text-slate-400" />{fmt(buy)}</>} />
        <Leg color="bg-teal-500" label={t('invest.legAppreciation')} value={<><DirhamSymbol size="0.7em" className="text-slate-400" />{fmt(a)}</>} />
        <Leg color="bg-emerald-500" label={t('invest.legRent')} value={<><DirhamSymbol size="0.7em" className="text-slate-400" />{fmt(r)}</>} />
      </div>
    </div>
  )
}
...
function TrendChart({ months, price, t }: { months: string[]; price: (number | null)[]; t: T }) {
  ...
  return (
    <div className="relative">
      ...
      <div className="mt-1 flex justify-between text-[10px] text-slate-400">
        ...
        <span className="font-semibold text-slate-600">{t('trend.latest', { value: Math.round(last).toLocaleString('en-US') })}</span>
