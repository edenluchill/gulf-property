...
// GET /investment?area=&property_type=&bedrooms=&offplan=
router.get('/investment', async (req: Request, res: Response) => {
  try {
    ...
    const ptype = String(req.query.property_type || 'apartment')
    const offplan = req.query.offplan === undefined || req.query.offplan === '' ? null : req.query.offplan === 'true'
    const r = await pool.query(
...
router.get('/report', async (req: Request, res: Response) => {
  try {
    ...
    const ptype = String(req.query.property_type || 'apartment')
    const r = await pool.query(
      'SELECT area_investment_report($1,$2,$3) AS data',
      [area, ptype, toBeds(req.query.bedrooms)]
    )
    res.json(r.rows[0].data)
  } catch (e: any) { res.status(500).json({ error: e.message }) }
})
...
router.get('/project-value', async (req: Request, res: Response) => {
  try {
    ...
    const rep = await pool.query('SELECT area_investment_report($1,$2,$3) AS d', [area, 'apartment', min_bedrooms])
    ...
    res.json({
      ...
      note: '报价 vs DLD 二手中位价(期房通常带溢价);仅按 min_bedrooms 公寓对标'
    })
  } catch (e: any) { res.status(500).json({ error: e.message }) }
})
...
router.get('/rent-vs-buy', async (req: Request, res: Response) => {
  try {
    ...
    const rep = await pool.query('SELECT area_investment_report($1,$2,$3) AS d', [area, ptype, toBeds(req.query.bedrooms)])
    const d = rep.rows[0].d
