...
// ============================================================================
// PDF Upload/Download Functions (for worker architecture)
// ============================================================================

/**
 * Upload PDF to R2 for worker processing
 *
 * @param buffer - PDF buffer
 * @param jobId - Job ID
 * @param filename - Original filename
 * @returns R2 key (not full URL) for storage in database
 */
export async function uploadPdfForProcessing(
  ...
  const key = `pending-pdfs/${jobId}/${filename}`;
...
export async function downloadFromR2(keyOrUrl: string): Promise<Buffer> {
  ...
  if (keyOrUrl.startsWith('https://')) {
    // Remove public URL prefix to get key
