-- ===========================================================================
-- 🪦 这条链路已于 2026-08-17 删除。下面是它 2026-07-11 上线时的原始迁移，留作历史。
--
-- 表 `developer_verifications`(1 行)和列 `lt_agents.developer_verified_at`(1 行)
-- **故意没有 DROP** —— 删掉只是为了删而删，留着不花钱，也是这段历史的唯一记录。
-- 删除理由(逐条查证，不是嫌麻烦)见 backend/src/routes/billing.ts 的墓碑注释：
--   徽章全站零渲染 / 它守的门是假的(role 自助就有) / 唯一通过的人交付 0 个楼盘 /
--   与后台通用的「赠 Pro 30 天」功能重复。
-- ===========================================================================
