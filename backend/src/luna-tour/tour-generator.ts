  ...
  return [
    ...
    isPublic
      ? [
          ...
          'Then `outro` (~9000ms): one sentence inviting a question, cta overlay. NO favorite_picker.',
          '`intro` ~18000ms ⭐ — the app puts you HIGH ABOVE the city and orbits slowly for',
          'exactly as long as you speak, so this is the one moment the viewer sees where in',
          'Dubai he actually is. Use it: name the project and its district, then say what',
          'that district IS and what famous thing it sits next to (from the `poi:`/`dist:`',
          'lines and the area name). Two or three unhurried sentences — a stranger to Dubai',
          'should end this beat able to point at the map. Do NOT list distances here (each',
          'neighbour gets its own beat later). Do NOT author camera moves — the app owns it.',
