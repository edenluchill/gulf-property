...
## 2026-06-07 编辑器重做为 NLE 风格时间线(真机:"不像编辑器,卡片要按时间线对齐")
- 把 `TourEditor` 从横向节点流重做成**视频剪辑软件式时间线**(纯前端,复用端点,无需部署):
  - **时间刻度尺**(秒,sticky)+ 缩放(px/s ±);整条 timeline 按 beat 真实时长(`seconds`)累加定位。
  - **多轨道**(共享时间轴):`停靠点`(彩色分段带 + ←→ 重排 / ✕ 删 / 末尾加地点)、`旁白`(每个 beat 一个片段,宽=时长)、`镜头`(每 beat 镜头摘要)、`卡片`(每张 overlay 按**绝对时间**[beat起点+at]定位的片段,宽=显示时长,多张分 lane)。
  - **卡片片段可拖动**:整体拖=移动 at_ms;拖右端=裁剪 duration_ms → POST `beat-overlays`(已支持 at_ms/duration)。所见即所得,与时间线对齐。
...
## 2026-06-07 简单模式(给非专业经纪 / 默认)
- 真机反馈:时间线对专业用户 OK,但 zoom 数字/多轨道/术语普通经纪看不懂;目标是非专业经纪也能自己生成(AI 也能操作)。
- **编辑器加双模式**(`TourEditor` `mode: 'simple'|'timeline'`,默认 simple,顶栏切换):
  - **简单模式 `SimpleView`**:大白话卡片列表(一屏一段画面,从上到下=播放顺序)。每段:房源/地点**缩略图** + 友好标题(开场介绍/登场亮相/生活与配套/投资数字/结尾邀约)+「这一段说的话」(直接改,失焦自动存)+「想怎么改→✨让 AI 改」(逐段)+「这一段会展示」友好 chips(房源信息卡/投资回报+X%/周边配套/实拍视频,带缩略图,✕ 去掉,➕ 加视频/图)+ 约 N 秒。停靠点表头可上移/下移/删除;段间「↓ 镜头飞到下一处」;末尾「➕ 加一站」地点搜索。**全程无术语、无 zoom 数字、无多轨道。**
  - **时间线模式**:保留专业 NLE(转场/停靠/旁白/镜头/卡片轨 + 拖拽 + 缩放)。
  - 两模式共用同一套端点(AI 与人同源)。
- 前端 tsc + build(本地 vite)+ 不变量 25/25。**纯前端,push 自动部署。**

## 2026-06-07 编辑器收敛(去双模式,卡片更易读)
- 真机反馈:别搞两个模式,只留时间线 + AI;节点要更易读、加图标;房源卡图没显示;进度点/点亮全部看不懂;"距离线"应显示"到哪里"。
- **改动**:
  - **删简单模式**,只留时间线(AI 改稿在右侧面板已具备)。
  - 卡片片段重做为**白底卡 + 左侧类型色条 + 图标**(🏠房源/📈ROI/📏距离/🌐配套/🎞媒体/🅣标题),房源卡/媒体显示**缩略图**(经代理),ROI 显示 **+X%**。
  - **隐藏系统卡**(进度点 progress_dots / 点亮全部 highlight_all_pins)——非作者内容。
  - **距离线显示目标**:后端用 overlay 自带 label → "到 地铁 0.4 km"(而非泛称"距离线")。
  - 侧栏卡片列表同步加图标/缩略图/隐藏系统卡。
- 核实:property_card 在 /script 带 image(r2.dev),图片代理 200 image/jpeg —— 旧版前端才不显示,新版会显示(无图回退 🏠 图标)。
- 前端 tsc + build + 不变量 25/25。后端(距离 label)需部署。

## 2026-06-07 编辑器:放大/高级 + 时长跟台词 + 内置预览
- 真机反馈:图片还是太小看不到;整体太小要更高级;改了台词上面的 20s 就不准;编辑器里要能播放。
- **改动**:
  - **放大更高级**:刻度尺 30px、停靠点带 54px(缩略图 h9×w12)、旁白轨 104px(13px 字/4 行)、镜头 38px、卡片片段 42px 高(图 h9×w14 → **房源图/媒体图终于显示**)+ 最小宽度(有图 110 / 无图 76)。侧栏同步加图标/缩略图。
  - **时长跟台词实时估算**:后端 flow `seconds = estimateSeconds(narration)`(CJK 4.2/s + latin 2.6词/s + 0.7 尾);前端 timeline `dur = estSec(narration)` **改台词即时更新**,显示 `~Ns`(估算,真值以重生成音频为准)。彻底解决"改词后 20s 不准"。
