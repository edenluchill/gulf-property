...
### 3.1 专项：1000 人同时（2026-06-27 补充）

区分三种"1000 人"，压力差几十倍：

| 场景 | 实际 RPS | 判断 |
|---|---|---|
| 1000 注册/试用，平时零散来 | 几~几十 | ✅ 毫无压力 |
| 1000 人这一分钟都在逛地图 | ~100-300 | ✅ 能撑，p95 上升但不崩 |
| 1000 人**同一秒**全冷启动涌入（营销推送） | 瞬时数百并发 | ⚠️ 零星延迟尖峰，大概率不崩 |

为什么 1000 安全：最重的负载源都已卸掉——前端/瓦片走 CDN 不占自有服务器；geojson 回访走 localStorage；**最重的 market 成交/租约聚合有缓存**（`market.ts:189-211`，内存 6h TTL + `market_cache` 表每日预热），基本不落 DB。单 cpx11 的瓶颈在「1万同时涌入」才危险，1000 低一个数量级，有余量。

唯一真实风险：1000 新用户同一秒冷启动首屏（拉 areas+landmarks+pois 三份大 geojson，且**未开 gzip**、单进程无 cluster）→ 几秒尖峰，部分用户首屏慢 1-2s，不会 502 雪崩。

消除办法（先做前两个，几乎免费）：①`app.use(compression())` 开 gzip，geojson 砍 70-80%；②geojson/静态接口加 `Cache-Control` 让 Cloudflare 边缘缓存。做完这俩，1000 人同一秒涌入也很稳。
